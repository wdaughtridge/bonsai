        ...
        case '\n':
            if (file != "") {
                ...
                if (type.find("directory") != std::string::npos) {
                    ...
                    std::string sessionName = executeWithOutput("tmux display-message -p '#S'").at(0); // get current tmux session name
                    size_t numPanes = executeWithOutput("tmux list-panes -t " + sessionName).size();
                    if (numPanes > 1)
                        system(std::string("cd " + dir + "; tmux send-keys -t " + sessionName + ".1 ':tabnew " + dir + "/" + file + "' Enter").c_str());
                    else
                        system(std::string("tmux split-window -p 75 -h -t " + sessionName + 
                                       " \"cd " + dir + "; nvim " + dir + "/" + file + "\"").c_str()); // split window in tmux session and open file in neovim
                }
            }
            break;
...
void Bonsai::printscr() {
    ...
    if (dir.size() + 3 <= width)
        ...
        std::string shortenedDir = dir.substr(0,firstSlash) + "/.." + dir.substr(lastSlash);
